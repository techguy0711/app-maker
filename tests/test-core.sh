...
SKILL="$1"                 # path to the PATCHED skill dir
...
bash "$SCRIPTS/doctor.sh" 2>&1 | check "unreachable probe -> WARN" "api.expo.dev is NOT reachable"
bash "$SCRIPTS/doctor.sh" 2>&1 | check "unreachable probe -> NONE verdict" "NONE — this shell cannot put a preview"
bash "$SCRIPTS/doctor.sh" 2>&1 | check "unreachable probe -> forbids token ask" "DO NOT ASK FOR AN EXPO ACCESS TOKEN"
EXPO_PROBE_URL=https://registry.npmjs.org/expo bash "$SCRIPTS/doctor.sh" 2>&1 \
...
OUT=$(EXPO_PROBE_URL=https://registry.npmjs.org/expo bash "$SCRIPTS/doctor.sh" 2>&1)
...
if [ -d "$PROJ/.claude/visual" ]; then
  ...
  R=$(cd "$PROJ" && CI=1 timeout 600 bash "$SCRIPTS/ui-validate.sh" 2>&1 | tail -1)
  grep -q "STATUS=pass" <<<"$R" && ok "CI=1 -> STATUS=pass (was: fail/0 forever)" || bad "CI=1 -> $R"
  R=$(cd "$PROJ" && CONTINUOUS_INTEGRATION=1 timeout 600 bash "$SCRIPTS/ui-validate.sh" 2>&1 | tail -1)
  grep -q "STATUS=pass" <<<"$R" && ok "CONTINUOUS_INTEGRATION=1 -> STATUS=pass" || bad "CONT_INT=1 -> $R"
  R=$(cd "$PROJ" && timeout 600 bash "$SCRIPTS/ui-validate.sh" 2>&1 | tail -1)
...
if [ -d "$PROJ" ]; then
  ...
  R=$(cd "$PROJ" && timeout 600 bash "$SCRIPTS/setup-visual-loop.sh" 2>&1 | tail -1)
  ...
  rm -rf "$PROJ/.claude/visual"; mkdir -p /tmp/empty-br-$$
  R=$(cd "$PROJ" && PLAYWRIGHT_BROWSERS_PATH=/tmp/empty-br-$$ timeout 600 bash "$SCRIPTS/setup-visual-loop.sh" 2>&1 | tail -1)
...
TSX
  ...
  R=$(timeout 900 bash "$SCRIPTS/setup-visual-loop.sh" 2>&1 | tail -1)
  ...
  R=$(CI=1 timeout 900 bash "$SCRIPTS/ui-validate.sh" 2>&1 | tail -1)
  grep -qE "STATUS=(pass|seeded)" <<<"$R" && ok "e2e: ui-validate under CI=1 -> $R" || bad "e2e: ui-validate -> $R"
  R=$(timeout 900 bash "$SCRIPTS/flow-validate.sh" 2>&1 | tail -1)
