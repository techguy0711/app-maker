...
SKILL="$(cd "${1:-$ROOT/plugins/app-maker/skills/app-maker}" && pwd -P)"
...
if node -e '
  ...
  if (p.name !== "app-maker" || p.skills !== "./skills/" ||
      p.interface?.displayName !== "App Maker" ||
...
if node -e '
  ...
  const p = m.plugins?.find(x => x.name === "app-maker");
  if (m.name !== "app-maker" || p?.source?.source !== "local" ||
      p?.source?.path !== "./plugins/app-maker" ||
...
if ruby -e '
  ...
  ok = d.dig("interface", "display_name") == "App Maker" &&
       d.dig("interface", "default_prompt").include?("$app-maker") &&
...
if ruby -e '
  ...
  exit(d["name"] == "app-maker" && !d.key?("version") &&
...
grep -q 'APP_MAKER_SKILL_DIR' "$SKILL/references/build-flow.md" \
