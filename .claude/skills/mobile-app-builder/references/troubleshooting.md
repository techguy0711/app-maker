...
```bash
<skill-base-dir>/scripts/make-preview-qr.sh <port> /tmp/preview-qr.png
```
(`<skill-base-dir>` = the absolute path shown as "Base directory for this
skill" when this skill was invoked — not a path relative to the project dir
you're likely `cd`ed into by this point.)
